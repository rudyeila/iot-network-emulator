...
def update_bandwidth(bw, delay=1):
	url = 'http://10.0.2.7:5000/topology/links?node1=left&node2=switch&bandwidth={}&delay={}'.format(bw, delay)
	payload = {}
	headers = {}
...
def run():
	...
	if result.error:
		...
		print('  Megabytes per second  (MBps)  {0}'.format(result.received_MB_s))
...
def main():
	...
	write_to_file("results.iperf", bandwidth, results)	
